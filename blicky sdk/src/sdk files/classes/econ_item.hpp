...
public:
	...
	/*void set_stattrak( int val ) { // NEEDS FIX PROBABLY
	...
	float get_paint_kit( ) {
		static float kit;
		get_attribute_value<float>( 6, kit );
		return kit;
	}
	...
	template<typename T>
	void set_attribute_value( int index, T val ) {
		auto v15 = ( DWORD* ) get_item_schema( );
	...
	template<typename T>
	void get_attribute_value( int index, T val ) {
		auto v15 = ( DWORD* ) get_item_schema( );
		auto v16 = *( DWORD* ) ( v15[ 72 ] + 4 * index );
